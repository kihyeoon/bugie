...
>
> **현재 상태**: 완료. 마이그레이션 2026-09-24 배포, 앱 1.5.0으로 2026-09-26 출시. 구글 신규 가입 실기기 항목(§7)은 체크하지 않았지만 사용자 확인에서 문제 없었다.
...
로그인 화면 ─ 애플 ─▶ 이름 정리 → rememberSignupName ─▶ signInWithIdToken ─▶ finally: 이름 비움
   ...
   └ 구글 ─▶ signInWithIdToken ─▶ 리스너 → ensureProfile(메타데이터 이름)
      ...
      login.tsx effect ─▶ 닉네임 화면 (이동은 여기 한 곳)
                 ▼
      시작하기: ① 기본 가계부 이름 변경 + refreshLedgers (해당할 때만, 실패해도 계속)
                ② profiles(full_name, onboarded_at) 저장 → 저장된 행으로 상태 갱신
                 ▼
...
- **가계부 이름을 먼저 바꾸고 프로필을 나중에 저장한다.** 프로필이 저장되는 순간 `needsProfile`이 false가 되어 이동하므로,
  순서를 반대로 하면 이동을 `saving` 같은 별도 상태로 붙잡아야 한다. 이름 변경 뒤 프로필 저장이 실패하면 가계부만 새 이름이
  되지만, 재시도하면 조건(`이전이름의 가계부`)에 안 걸려 다시 바뀌지 않으므로 무해하다.
...
- [ ] 프로필 설정 닉네임 수정: 가계부 이름은 그대로이고(09-25 실기기 확인), 실패하면 알림이 한 번만 뜬다(미확인)
- [x] 로그인한 상태에서 `bugie://invite?code=…`로 연 미완료 사용자: 닉네임 → 초대 수락 화면

**실기기**
- [x] 새 애플 가입: 이름이 기본값으로 채워지고 가계부 이름이 맞다 (09-25 TestFlight 빌드 21, 프라이빗 이메일)
  (설정 › Apple ID › 애플로 로그인에서 연결을 끊고, dev-account-deletion 절차를 거친다)
- [x] 애플 재로그인(이름 없음): `''` 이름이 저장되지 않는다 (09-25 실기기. 기본값 빈칸, 저장 시 가계부 이름 변경 확인)
