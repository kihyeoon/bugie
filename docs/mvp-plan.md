...
- [x] 가계부 전환 기능
- [x] 가계부 설정 이동
- [ ] **멤버 관리 기능** (가계부 관리 내 포함)
  - [ ] 멤버 목록 화면
    - [ ] 현재 멤버 리스트 표시
    - [ ] 멤버별 권한 표시 (owner/admin/member/viewer)
    - [ ] 프로필 사진 및 이름
    - [ ] 멤버 삭제 기능 (권한에 따라)
  - [ ] 멤버 초대 화면
    - [ ] 이메일 입력 필드
...
8. **거래 상세/수정 화면** - 네이티브로 구현, 인라인 편집 모달 완료
   - 5개 편집 모달 구현 (금액, 카테고리, 제목, 메모, 날짜)
   - Soft Delete 기능 및 RLS 충돌 해결
   - 화면 간 데이터 동기화 구현
...
- **UI 구현**: 70% (인증, 탭 네비게이션, 홈, 빠른 입력, 거래 목록, 거래 상세)
...
- ✅ 거래 목록 화면 구현 (transactions.tsx)
- ✅ 스크롤 반응형 캘린더 (월간 ↔ 주간 뷰 전환)
- ✅ 캘린더-리스트 양방향 동기화 (디바운스 적용)
- ✅ **거래 상세 화면 구현** (transaction-detail.tsx)
- ✅ **인라인 편집 모달 5종 구현** (금액, 카테고리, 제목, 메모, 날짜)
- ✅ **거래 삭제 기능** (Soft Delete + SECURITY DEFINER)

#### 기술적 개선

- ✅ useTransactions 훅 구현 (데이터 페칭 및 캐싱)
- ✅ **useFocusEffect로 화면 간 데이터 동기화**
- ✅ **RLS와 Soft Delete 충돌 문제 해결**
- ✅ 타이밍 유틸리티 추가 (debounce, throttle)
- ✅ **모달 애니메이션 최적화** (spring animation)
- ✅ **키보드와 모달 동시 애니메이션 구현**
